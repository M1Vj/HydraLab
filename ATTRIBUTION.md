...
| Durable workflow dependency for branch 02-03 | none bundled | none | reference-only | No dependency added. | cleared | Branch 02-03 decision (2026-07-02): implemented the bounded seven-stage runner as HydraLab-owned code in `backend/hydra/orchestrator/` rather than adopting LangGraph or another durable-workflow package. |
| AutoResearchClaw (`aiming-lab/AutoResearchClaw`) | MIT | none | reference-only | Studied only; no source copied, vendored or adapted. | cleared-reference | Branches 03-01 and 03-02 reference for staged closed-loop research/autonomy workflow shape, checkpoint/co-pilot concepts, claim-verification inspiration and self-evolving loop concepts. HydraLab reimplements with owned code. |
| Co-Scientist paper (`arXiv:2502.18864`) | CC-BY-4.0 | none | reference-only | Studied only; no source copied, vendored or adapted. | cleared-reference | Branch 03-02 reference for supervisor, generation/reflection/ranking/evolution/meta-review orchestration patterns. HydraLab implements owned `AdvancedRunConfig` and safety-bound run controls. |
| Robin paper (`arXiv:2505.13400`) | CC-BY-4.0 | none | reference-only | Studied only; no source copied, vendored or adapted. | cleared-reference | Branch 03-02 reference for iterative literature, hypothesis, experiment-planning, ranking and update-loop concepts. No Robin code copied. |
| Robin repository (`Future-House/robin`) | Apache-2.0 | none | reference-only | Studied only; no source copied, vendored or adapted. | cleared-reference | Branch 03-02 reference for multi-agent discovery workflow shape and candidate/ranking lifecycle. No dependency added. |
| MLEvolve (`InternScience/MLEvolve`) | unknown / verify before adoption | unknown | reference-only | Studied only; no source copied, vendored or adapted. | cleared-reference | Branch 03-02 reference for population search, experience memory and budgeted evolution ideas deferred from execution surfaces. No dependency added. |
| OpenAI Codex approval-model surface | proprietary service/docs reference | none as reference | reference-only | UX/process reference only; no source copied or dependency added. | cleared-reference | Branch 03-01 reference for approval/gate UI patterns in autonomous coding surfaces. HydraLab keeps DEC-6 no-arbitrary-code-execution and implements its own approval/checkpoint/audit contracts. |
...
| Playwright Python | Apache-2.0 | none | bundled-dependency | Permissive; ships freely; browser binaries remain separately installed runtime assets. | cleared | Branch 03-06 autonomous browser research driver uses headed Playwright for public/local pages, with offline fake-driver tests so CI never requires a live browser or network. No stealth/anti-detect behavior is implemented. |
...
| Hermes (Nous Research, `hermes-agent.nousresearch.com/docs`) | (docs, verify at any adoption) | none | reference-only | Documentation studied; no code vendored. | cleared | Branch 03-05 studied Hermes' skill/memory/self-host model for the self-evolution change-set idea, then reimplemented it entirely through HydraLab-owned contracts (03-01 checkpoint/audit shell + the fixed verification allowlist). No Hermes code or data vendored. |
| Pi (`github.com/earendil-works/pi`) | (verify at any adoption) | none | reference-only | Markdown-agent/skill pattern studied; no code vendored. | cleared | Branch 03-05 studied Pi's markdown-defined agents/skills as a mental model for typed skill/prompt diffs; the HydraLab skill descriptor + risk classifier are independently implemented. No Pi code vendored. |
| OpenAI Codex approve-before-apply (`developers.openai.com/codex/app/chrome-extension`) | (docs) | none | reference-only | UX workflow studied; no code vendored. | cleared | Branch 03-05 studied Codex's approve-before-apply code-modification workflow to shape the propose → explicit approval → checkpoint → apply → auto-verify → keep-or-rollback gate; reimplemented over HydraLab's own services. No Codex code vendored. |
