...
class CheckpointError(RuntimeError):
    """Raised when no restorable ref can be recorded for a checkpoint."""
    ...
    async def create(
        ...
        *,
        ...
        if git_result:
            ...
            # Clean tree: git.checkpoint() creates no commit. Pin the CURRENT
            # HEAD so the pre-write state stays restorable to a fixed sha rather
            # than a moving "HEAD" ref (M3).
            head = getattr(self.git, "head_commit", None)
            commit = str(head() or "") if callable(head) else ""
            git_ref = commit
        if not commit:
            raise CheckpointError(
                "no restorable ref recorded; refusing to report a checkpoint for a high-risk write"
            )
        row = AgentCheckpoint(
